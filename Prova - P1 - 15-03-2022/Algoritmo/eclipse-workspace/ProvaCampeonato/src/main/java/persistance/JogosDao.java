...
public class JogosDao implements IJogosDao {
	...
	public void sorteiaJogos() throws SQLException, ClassNotFoundException {
		...
		String sql_query2 = "SELECT * FROM Jogos WHERE ((codigoTimeA = ? OR codigoTimeB = ?) OR (codigoTimeA = ? OR codigoTimeB = ?)) AND data = ?";
		...
		while(cont1 < 12) {
			jogos.clear();
			jogos.addAll(jogos_backup);
			jogos.removeAll(jogos_marcados);
			while(cont2 < 8) {
				int index = aleatorio.nextInt(jogos.size()) + 1;
